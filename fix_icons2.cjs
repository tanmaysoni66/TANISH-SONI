...
// The issue was that when inserting the custom icons, they were placed before the imports. 
// Let's move them inside the App component, or after the imports, but we used a dirty replace.
// Wait, the icons were inserted *before* `const App = () => {`. That's outside the React component. That is fine, but React needs to be in scope. `import React from 'react';`
// But wait, the error is in SSR (Server Side Rendering). `PinterestIcon is not defined`.
// Oh! Vite builds the client and server differently. 
// Let's just put the definitions inside the file properly.

code = code.replace(/const PinterestIcon = \(\{ size = 24, color = "currentColor", strokeWidth = 2, \.\.\.props \}: any\) => \([\s\S]*?<\/svg>\n\);/g, '');
code = code.replace(/const TelegramIcon = \(\{ size = 24, color = "currentColor", strokeWidth = 2, \.\.\.props \}: any\) => \([\s\S]*?<\/svg>\n\);/g, '');
code = code.replace(/const QuoraIcon = \(\{ size = 24, color = "currentColor", strokeWidth = 2, \.\.\.props \}: any\) => \([\s\S]*?<\/svg>\n\);/g, '');
...
const insertIndex = code.indexOf('const Footer = () => {');
code = code.substring(0, insertIndex) + customIcons + '\n\n' + code.substring(insertIndex);

fs.writeFileSync('src/App.tsx', code);
console.log('Fixed icon scope.');
